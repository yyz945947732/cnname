...
describe('cnname.options.unique', () => {
  ...
  test('cnname({ count: 5, unique: true, returnType: "surname", surname: "欧阳" }) should return array with one elements', () => {
    const result = cnname({ count: 5, unique: true, returnType: 'surname', surname: '欧阳' });
    const [name] = result;
    expect(result).toHaveLength(1);
    expect(name).toBe('欧阳');
  });
  test('cnname({ count: 5, unique: true, returnType: "surname", surname: ["李", "张"] }) should return array with one elements', () => {
    const result = cnname({ count: 5, unique: true, returnType: 'surname', surname: ['李', '张'] });
  ...
  test('cnname({ count: 5, unique: true, returnType: "givenName" }) should return array with givenName', () => {
    expect(cnname({ count: 5, unique: true, returnType: 'givenName' })).toHaveLength(5);
  });
  test('cnname({ count: 5, unique: true, returnType: "surname" }) should return array with surname', () => {
    expect(cnname({ count: 5, unique: true, returnType: 'surname' })).toHaveLength(5);
  });
});
